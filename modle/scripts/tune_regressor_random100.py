...
TRIAL_LOG_FILE = ARTIFACTS_DIR / "regressor_random100_trials.csv"
SUMMARY_FILE = ARTIFACTS_DIR / "regressor_random100_summary.md"
GRAPH_FILE = ARTIFACTS_DIR / "regressor_random100_ndcg_progress.png"
...
class TuningContext:
    ...
    augmented_history: pd.DataFrame
    target: pd.Series
...
def build_tuning_context() -> TuningContext:
    ...
    features = history[mr.FEATURE_COLUMNS]
    target = history["방문인원수"]
    train_medians = features.median(numeric_only=True)
    filled_features = features.fillna(train_medians)
    classifier = CatBoostClassifier(**mr.CLASSIFIER_PARAMS)
    classifier.fit(filled_features, runtime._make_stage1_busy_labels(target))
    augmented_history = runtime._append_stage1_outputs(classifier, filled_features)
    sample_weight = runtime._make_busy_sample_weights(target)
...
def predict_validation_period(context: TuningContext, regressor: CatBoostRegressor) -> pd.DataFrame:
    ...
    while current_date <= end_date:
        ...
        filled_features = row_frame[mr.FEATURE_COLUMNS].fillna(context.train_medians)
        stage1_augmented = context.runtime._append_stage1_outputs(context.classifier, filled_features)
...
def evaluate_trial(spec: TrialSpec, context: TuningContext) -> dict[str, Any]:
    regressor = CatBoostRegressor(**spec.params)
    regressor.fit(context.augmented_history, context.target, sample_weight=context.sample_weight)

    prediction_frame = predict_validation_period(context, regressor)
...
def build_trial_specs() -> list[TrialSpec]:
    ...
    spaces = {
        ...
        "l2_leaf_reg": [2.0, 3.0, 5.0, 8.0, 10.0, 15.0],
    }
    current = (
        mr.REGRESSOR_PARAMS["depth"],
        mr.REGRESSOR_PARAMS["learning_rate"],
        mr.REGRESSOR_PARAMS["iterations"],
        mr.REGRESSOR_PARAMS["l2_leaf_reg"],
    )
...
def save_graph(results: list[dict[str, Any]]) -> None:
    ...
    plt.title("Regressor Random Search 100 Progress")
